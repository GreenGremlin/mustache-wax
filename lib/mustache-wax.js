...
function MustacheWax(argv, fromCli) {
...
MustacheWax.prototype = {
    ...
    processTemplate: function (template, root) {
        ...
        if (stat.isDirectory()) {
            ...
            try {
                precompiled = fs.readFileSync(tmpl, 'utf8');
            } catch (ex) {
                throw ex;
            }

            partials = this.partialsParser(precompiled);

            precompiled = Handlebars.precompile(precompiled, {
            ...
            if (this.argv.simple) {
                ...
                this.output.push(this.wrapModule(template, precompiled, partials));
            }
        }
    },

    // from https://github.com/yahoo/locator-handlebars/blob/master/lib/formats/yui.js
    partialsParser: function (source) {
        var parsed = Handlebars.parse(source),
            partials = [];
    ...
    wrapModule: function (templateName, precompiledFn, partials) {
        // base dependency
        ...
        partials.forEach(function (partialName) {
            ...
            dependencies.push(this.prefix + partialName);
        });
        ...
        return this.templateFn({
            ...
            ns: this.namespace,
            partials: partials,
            hasPartials: partials.length > 0,
            stringifiedPartials: JSON.stringify(partials),
